...
model <- function(x){
  ...
  SimulateModel(end_week = end_week, 
                ...
                I0 = I0, 
                seasonality = c(FALSE),
                seasonality_beta = x[(2)],
                essential_pars = x[c(c(3,4,5))],
                detect_pars  = x[6], 
                susc_infect_pars = x[7], 
                unitData = unitData, 
                dt = 1/10,
                trans.mode = "dens", 
                forest_unit = "decimal", 
                detect = "constant", 
                susc_infect = "single constant")
...
if (!any(list.files(file.path(scen_folder_wd, folder_posterior_points)) %in% "posterior_points.RDS")){
  ...
  foreach(i = 1:(nb_simul/2), .combine = rbind, .packages = c("tidyverse") , .export = "scen_folder_wd") %dopar% {
    ...
    names(pars) = c("transmission rate", 
                    "detection rate",
                    "sanitation rate", 
                    "recovery rate",
                    "detectibility", 
    ...
    SimulateModel(end_week = end_week, 
                  ...
                  I0 = I0, 
                  seasonality = c(FALSE),
                  seasonality_beta = pars[1],
                  essential_pars = pars[2:4],
                  detect_pars  = pars[c(5)], 
                  susc_infect_pars = pars[c(6)], 
                  unitData = unitData, 
                  dt = 1/10,
                  trans.mode = "dens", 
                  forest_unit = "decimal", 
...
if (!any(list.files(paste0(scen_folder_wd,"/Trajectories/" )) %in% "simulated_trajectories.RDS")){
  ...
  foreach(i = 1:traj_simul, .combine = rbind, .packages = c("tidyverse"), .export = c("post_ABC")) %dopar% {
    ...
    source(("/Model/cal_sum_stats.sim_ver3_extended.R"))
    source(("/Model/Simulate_model_wb_general_tracked.R"))    
    source(("/Model/gen.matsum.sim.R"))
    load("initdata.Rdata", .GlobalEnv)
    ...
    SimulateModel(end_week = end_week, 
                  ...
                  seasonality = c(FALSE),
                  seasonality_beta = pars[(1)],
                  essential_pars = pars[c(2:4)],
                  detect_pars  = pars[5], 
                  susc_infect_pars = pars[6], 
                  unitData = unitData, 
                  dt = 1/10,
                  trans.mode = "dens", 
                  forest_unit = "decimal", 
    ...
    tracked_data %>% group_by(To) %>% dplyr::select(To, Time) %>% arrange(To)  -> dtf_date_infection
    
    tracked_data %>%  distinct(To) %>% arrange(To)%>% pull ->ind_infector
    
    l_R_each = list(NA)
    
    
    for(i in seq_along(ind_infector)) {
      
      dtf_date_infection %>% filter(To == ind_infector[i]) -> infector_time
      
      
      if(nrow(infector_time) == 1) {
        
        R <- tracked_data %>% filter(From == infector_time$To) %>% nrow()
        ...
        for(j in 1:c(nrow(infector_time)-1)){
          
          
          
          R[j] <-tracked_data %>% filter(From == infector_time[j,1] %>%pull,
                                         Time >= infector_time[j,2] %>% pull & Time < infector_time[c(j+1),2]%>% pull) %>% nrow()
          
          
        }
        
        
        R[c(j+1)] <- tracked_data %>% filter(From == infector_time[nrow(infector_time),1] %>% pull, 
                                             Time >= infector_time[nrow(infector_time),2] %>% pull) %>% nrow()
        
        
        
        
      }
      l_R_each[[i]]<-  dtf_results <- data.frame(From = infector_time[[1]], time = infector_time[[2]],R = R )
    ...
    return(list(sim_same_par_traj_reported,
                ...
                sim_same_par_traj_inci_undetected,sim_same_par_traj_inci_to_be_detected, table_num_reinfect, matr.states, tracked_data, l_R_each))
...
list_trajec[,1:7] %>% apply(MARGIN = 2, FUN = function(x){
...
sim_same_par_traj_reported = readRDS("/data/sim_same_par_traj_reported.rds")
...
uni_sf_hex = sf_hex %>% st_union

tm_shape(tm_kor,bbox =hex_bbox) +
  tm_polygons(col = "grey80",lwd = 1) +
tm_shape(sf_hex) +
  tm_fill("risk_ob", title = "Prob. of ASF detection",
          style = "cont"
  ) + 
  tm_shape(NK_map) +
  tm_fill(fill = "grey60")+
  ...
  tm_layout(frame = FALSE, 
            legend.frame = FALSE, 
            legend.position = c("right","bottom"))-> risk_ob_plot;risk_ob_plot
...
tm_shape(tm_kor,bbox =hex_bbox) +
  tm_polygons(col = "grey80",lwd = 1) +
  tm_shape(sf_hex) +
  tm_fill("risk_inci", title = "Risk of ASF occurrences",
          style ="cont", 
  ...
  tm_layout(frame = FALSE, 
            legend.frame = FALSE,
            legend.position = c("right","bottom")) -> risk_inci_plot
...
setwd("/post_hoc_analysis/")
brakes = c(30, 130, 330, 530, 730, 930, 1100)
...
tm_shape(tm_kor,bbox =hex_bbox) +
  ...
  tm_borders(lwd = 0.5,
             col = "grey30") + 
  tm_shape(sf_mydata) +
  tm_dots(fill = "day_death", 
          size = 0.4, 
          fill.scale = tm_scale_continuous(values  =  "brewer.rd_yl_bu", 
                                    ticks = brakes, 
                                    labels = as.character(as.Date("2019-09-01") + brakes)),
  ...
  ) -> surv_database;surv_database

pdf(file = "study_region.pdf",   # The directory you want to save the file in
    width = 10, # The width of the plot in inches
    height = 10) 
...
plot(NK_map)
?st_combine
tm_penni = st_union(tm_kor, NK_map)
...
pdf(file = "korea_pennin.pdf",   # The directory you want to save the file in
...
pdf(file = "surv_database.pdf",   # The directory you want to save the file in
    width = 10, # The width of the plot in inches
    height = 7) 
...
setwd((file.path(scen_folder_wd, "/Diagnostic")))
jpeg(paste0("ROC_curve.jpeg"), width = 800, height = "800", quality = 100)
