...
setwd("/Results/")
folder_scen = paste0("Model_J",scen_num)
...
pars = c(0.07, 5/2, 0.37, 2 ,1/4.66, 1/5.88, 0.073, 0.2,0.5, 1)
...
model <- function(x){
  ...
  SimulateModel(end_week = end_week, 
                ...
                detect_pars  = x[c(9,10)], 
                susc_infect_pars = x[11], 
                unitData = unitData, 
                dt = 1/10,
                trans.mode = "dens", 
                forest_unit = "decimal", 
...
if (!any(list.files(file.path(scen_folder_wd, folder_posterior_points)) %in% "posterior_points.RDS")){
  ...
  foreach(i = 1:(nb_simul/2), .combine = rbind, .packages = c("tidyverse") , .export = "scen_folder_wd") %dopar% {
    ...
    SimulateModel(end_week = end_week, 
                  ...
                  detect_pars  = pars[c(8, 9)], 
                  susc_infect_pars = pars[c(10)], 
                  unitData = unitData, 
                  dt = 1/10,
                  trans.mode = "dens", 
                  forest_unit = "decimal", 
