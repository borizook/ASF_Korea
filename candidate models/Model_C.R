...
setwd("/Results/")
folder_scen = paste0("Model_C",scen_num)
...
pars_estimate = c("transmission rate", 
                  ...
                  "rel_suscep", 
                  "rel_infect")
pars = c(0.09, 1/4.66, 1/5.88, 0.073, 0.5, 1, 1)
...
if (!any(list.files(paste0(scen_folder_wd,"/Trajectories/" )) %in% "simulated_trajectories.RDS")){
  ...
  foreach(i = 1:traj_simul, .combine = rbind, .packages = c("tidyverse"), .export = c("post_ABC")) %dopar% {
    ...
    source(("/model/cal_sum_stats.sim_ver3_extended.R"))
    source(("/model/Simulate_model_wb_general.R"))    
    source(("/model/gen.matsum.sim.R"))load("initdata.Rdata", .GlobalEnv)
...
hex <- readRDS("/save_home/jslim/Mechanistic_modelling/Marryland/data/RDS/hexagon.RDS")
library(sf)
hex %>% st_as_sf ->sf_hex
