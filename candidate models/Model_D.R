...
setwd("/Results/")
folder_scen = paste0("Model_D",scen_num)
...
priors <- list(
  ...
  c("unif", 0, 1),   # 9.  Maximum detectibility
  c("unif", 0, 1),   # 9.  Slope of detectibility
  c("unif", 1, 1)    # 9.  relative susceptibility/infectibility of cell with zero forest density (should be between 0 - 1)
...
pars_estimate = c("transmission rate", 
                  "Maximum detectibility",
                  "Slope of detectibility")
pars = c(0.09, 1/4.66, 1/5.88, 0.073, 0.5, 0.5, 1)
...
if (!any(list.files(file.path(scen_folder_wd, folder_posterior_points)) %in% "posterior_points.RDS")){
  ...
  foreach(i = 1:(nb_simul/2), .combine = rbind, .packages = c("tidyverse") , .export = "scen_folder_wd") %dopar% {
    ...
    source(("/model/cal_sum_stats.sim_ver3_extended.R"))
    source(("/model/Simulate_model_wb_general.R"))
    source(("/model/gen.matsum.sim.R")) setwd(paste0(file.path(scen_folder_wd,"/init_data" )))
    ...
    SimulateModel(end_week = end_week, 
                  ...
                  detect_pars  = pars[c(5, 6)], 
                  susc_infect_pars = pars[c(7)], 
                  unitData = unitData, 
                  dt = 1/10,
                  trans.mode = "dens", 
                  forest_unit = "decimal", 
